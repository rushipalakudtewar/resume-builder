...
import { v4 } from 'uuid';
import Resume2 from './resume-preview/Resume2';
...
useEffect(()=>{
  ...
  <>
      <div className='flex justify-between items-center mx-0 md:mx-64 my-0 md:my-10 '>
        ...
        <div className='mr-14'>
            <button onClick={handleGeneratePdf} className='text-white bg-gradient-to-r from-cyan-500 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 shadow-md mr-2'>Save as PDF</button>
            ...
            {downloadLink && <button className='text-white bg-gradient-to-r from-cyan-500 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 shadow-md mr-2' onClick={generateShareableLink}>Share</button>}
            <ShareModel isOpen={isModalOpen} onClose={closeModal} copiedUrl={shareableLink} />
        </div>
    </div>
  <div className="mx-0 md:mx-60 my-0 md:my-10 bg-gray-200 p-4 m-2 w-[794px]  border-gray-800 shadow-lg rounded-md">
    <div ref={templateRef}>
        <Resume2 dataStore={dataStore} userPhoto={userPhoto} isLoading={isLoading}/>
        </div>
    </div>
  </>
  )
